      ...
      if (!block) {
        ...
        for (let shard of streamerMessage.shards) {
          if (shard.chunk !== undefined && shard.chunk !== null) {
              ...
              if (filteredTrxns.length > 0) {
                ...
                for (let transaction of filteredTrxns) {
                  const action = JSON.parse(
                    JSON.stringify(transaction.transaction.actions[0])
                  );
            ...
            //   }
            // })
            let pendingReceipts = nearResponses.filter(
              (x) => x.status === ResponseStatus.PENDING
            );
            ...
            //   }
            // }
            let filteredReceipts = shard.receiptExecutionOutcomes.filter((x) =>
              pendingReceipts.some((item) =>
                item.receiptIds?.includes(x.executionOutcome.id)
              )
            );
            ...
            if (filteredReceipts.length > 0) {
              ...
              for (let receipt of filteredReceipts) {
                ...
                //   }
                // })
                let pending = pendingReceipts.find(
                  (x) =>
                    x.receiptIds !== undefined &&
                    x.receiptIds?.includes(receipt.executionOutcome.id) &&
                    x.status === ResponseStatus.PENDING
                );

                //get the last element of receiptIds array
                if (
                  pending !== undefined &&
                  pending.receiptIds !== undefined &&
                  pending.receiptIds[pending.receiptIds.length - 1] ===
                    receipt.executionOutcome.id
                ) {
                  nearBlocks.push(
                        ...
                        */
                        ...
                        if (!gameTeam) {
                          ...
                          if (game) {
                            ...
                            let token_id: string[] =
                              args.token_ids !== undefined &&
                              args.token_ids !== null
                                ? args.token_ids
                                : [];
                            let promo_ids: string[] =
                              args.token_promo_ids !== undefined &&
                              args.token_promo_ids !== null
                                ? args.token_promo_ids
                                : [];
                        ...
                        if (!game) {
                          ...
                          Logger.debug(
                            `Game ${args.game_id} created for ${SportType.MLB}`
                          );
                        } else {
            ...
            if (event.event.event === "lineup_submission_result") {
              ...
              if (game) {
                ...
                if (!gameTeam) {
                  ...
                  Logger.error(
                    `Team already exist on Game ${game.gameId} for ${game.sport}`
                  );
                }
              } else {
                Logger.error(
              ...
              if (game) {
                ...
                Logger.debug(
                  `Game ${event.event.data[0].game_id} created for ${SportType.MLB}`
                );
                let nearBlock = await NearBlock.create({
                  height: event.block_height,
                  hash: event.block_hash,
